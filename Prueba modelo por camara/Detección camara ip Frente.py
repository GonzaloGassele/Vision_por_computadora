...
cap = cv2.VideoCapture('rtsp://contralor:Villegas555@100.100.34.184/cgi-bin/main.cgi') #variable con la direccion de la camara
w=int(cap.get(3)/2)
h=int(cap.get(4)/2)
...
while(True):
    ...
    if frame is None: #si hay frame vacio 
        cap = cv2.VideoCapture('rtsp://contralor:Villegas555@100.100.34.184/cgi-bin/main.cgi')
    else:
        img = cv2.resize(frame,(0,0),fx=0.5, fy=0.5)
        result= model(img)
        result.render()
    ...
    cv2.imshow('Camera frente ingreso del Polo',img)  #muestra la camara ip
